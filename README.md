...
在 Windows 环境完成 `npm run make:win` 打包或发布 `v2.0.1` 后，可使用生成的 `Terminal-Agent-Setup-2.0.1.exe` 安装包；发布后也可从 [Releases](https://github.com/AI-DIY/Terminal-Agent/releases) 获取。安装完成后，先启动一次 Terminal-Agent。完整的安装、`putty.exe` 替换和堡垒机跳转步骤见 [RELEASE_NOTES.md](RELEASE_NOTES.md)。
...
在 Windows 环境完成 `npm run make:win` 打包或发布 `v2.0.1` 后将生成以下文件：

| 文件 | 用途 |
| --- | --- |
| `Terminal-Agent-Setup-2.0.1.exe` | Windows x64 安装包。 |
| `putty.exe` | 提供给 Assess/Access Client 或堡垒机映射的单文件桥接程序。 |
| `Terminal-Agent-Uninstall-Cleanup-2.0.1.zip` | 清理“已安装的应用”中遗留 Terminal-Agent 卸载条目的工具。它不会卸载程序、删除应用文件或删除用户数据。 |
