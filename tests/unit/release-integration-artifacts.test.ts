...
describe('release integration artifact guard', () => {
  it('keeps v3.3.6 package metadata, installer artifact naming, and release notes synchronized', () => {
    ...
    expect(packageLock.version).toBe('3.3.6')
    expect(packageLock.packages[''].version).toBe('3.3.6')
    ...
    expect(releaseNotes.startsWith('# Terminal-Agent v3.3.6')).toBe(true)
    expect(releaseNotes).toContain('Terminal-Agent-Quick-Install-3.3.6.zip')
    expect(releaseNotes).toContain('Terminal-Agent-Uninstall-Cleanup-3.3.6.zip')
