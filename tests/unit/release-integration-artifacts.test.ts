...
describe('release integration artifact guard', () => {
  it('keeps v3.2.8 package metadata, installer artifact naming, and release notes synchronized', () => {
    ...
    expect(packageLock.version).toBe('3.2.8')
    expect(packageLock.packages[''].version).toBe('3.2.8')
    ...
    expect(releaseNotes.startsWith('# Terminal-Agent v3.2.8')).toBe(true)
    expect(releaseNotes).toContain('Terminal-Agent-Quick-Install-3.2.8.zip')
    expect(releaseNotes).toContain('Terminal-Agent-Uninstall-Cleanup-3.2.8.zip')
