...
describe('release integration artifact guard', () => {
  it('keeps v3.3.7 package metadata, installer artifact naming, and release notes synchronized', () => {
    ...
    expect(packageLock.version).toBe('3.3.7')
    expect(packageLock.packages[''].version).toBe('3.3.7')
    ...
    expect(releaseNotes.startsWith('# Terminal-Agent v3.3.7')).toBe(true)
    expect(releaseNotes).toContain('Terminal-Agent-Quick-Install-3.3.7.zip')
    expect(releaseNotes).toContain('Terminal-Agent-Uninstall-Cleanup-3.3.7.zip')
