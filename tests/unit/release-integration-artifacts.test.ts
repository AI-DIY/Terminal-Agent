...
describe('release integration artifact guard', () => {
  it('keeps v3.3.4 package metadata, installer artifact naming, and release notes synchronized', () => {
    ...
    expect(packageLock.version).toBe('3.3.4')
    expect(packageLock.packages[''].version).toBe('3.3.4')
    ...
    expect(releaseNotes.startsWith('# Terminal-Agent v3.3.4')).toBe(true)
    expect(releaseNotes).toContain('Terminal-Agent-Quick-Install-3.3.4.zip')
    expect(releaseNotes).toContain('Terminal-Agent-Uninstall-Cleanup-3.3.4.zip')
