...
describe('Terminal-Agent branding', () => {
  it('packages one stripe-free TA mark across native and workbench surfaces', () => {
    ...
    expect(packageJson.version).toBe('3.3.6')
    expect(packageLock.version).toBe(packageJson.version)
    expect(packageLock.packages[''].version).toBe(packageJson.version)
    expect(packageJson.build.nsis.artifactName.replace('${version}', packageJson.version).replace('${ext}', 'exe')).toBe('Terminal-Agent-Setup-3.3.6.exe')
  ...
  it('names the side regions as task history and AI workspace with directional expand actions', () => {
    ...
    expect(shell).toContain('展开任务空间')
    expect(shell).toContain('展开 AI工作区')
    expect(shell).toContain('调整任务空间宽度')
