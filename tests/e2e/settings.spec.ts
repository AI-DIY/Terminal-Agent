...
test('opens real settings with the user-facing panels and preserves drafts on return', async () => {
  ...
  try {
    ...
    await expect(panels).toHaveText(['大语言模型配置', '外观', '单点登录'])
    await expect(page.getByRole('button', { name: '安全围栏', exact: true })).toHaveCount(0)
    await expect(page.getByRole('heading', { name: '安全围栏', exact: true })).toHaveCount(0)
