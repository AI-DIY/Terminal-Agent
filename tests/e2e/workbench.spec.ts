...
test('restores a newly created zero-SSH chat as a connectable live workspace after reload', async ({ launchApp }) => {
  ...
  try {
    ...
    await page.getByRole('button', { name: '新建任务', exact: true }).click()
    const activeChat = page.locator('.history-item.active')
    const title = (await activeChat.locator('.chat-title > strong').textContent())?.trim()
    if (!title) throw new Error('Expected a durable chat title')
    const chatId = await activeChat.getAttribute('data-chat-id')
    if (!chatId) throw new Error('Expected a durable chat id')
...
test('restores an empty live workspace while another chat keeps a running SSH session', async ({ launchApp }) => {
  const sshServer = await startSshServer()
  let app: ElectronApplication | undefined
...
test('restores a chat with only closed SSH sessions to the new-SSH empty state after reload', async ({ launchApp }) => {
  ...
  try {
    ...
    await expect(page.getByRole('button', { name: '选择终端会话 127.0.0.1', exact: true })).toHaveCount(0)
    const title = (await activeChat.locator('.chat-title > strong').textContent())?.trim()
    if (!title) throw new Error('Expected a durable task title after its first SSH association')
    const chatId = await activeChat.getAttribute('data-chat-id')
    if (!chatId) throw new Error('Expected a durable chat id')
...
test('transfers a running SSH session before deleting its live chat', async ({ launchApp }) => {
  const sshServer = await startSshServer()
  let app: ElectronApplication | undefined
  ...
  try {
    ...
    await page.reload()
    await waitForWorkbenchReady(page)
    await chatItem(page, transferredChatId).getByRole('button', { name: /^选择任务 / }).click()
    ...
    await page.getByRole('button', { name: '关闭 SSH 会话 127.0.0.2', exact: true }).click()
...
test('restores authoritative multi-chat session ownership and fallback layouts', async ({ launchApp }) => {
  ...
  try {
    ...
    await page.getByRole('button', { name: '新建任务', exact: true }).click()
    await connect(page, sshServer.port, '127.0.0.1')
    const firstChatId = await page.locator('.history-item.active').getAttribute('data-chat-id')
    if (!firstChatId) throw new Error('Expected first task id')

    await page.getByRole('button', { name: '新建任务', exact: true }).click()
    await connect(page, sshServer.port, '127.0.0.2')
    await connect(page, sshServer.port, '127.0.0.3')
    const secondChatId = await page.locator('.history-item.active').getAttribute('data-chat-id')
    if (!secondChatId) throw new Error('Expected second task id')
...
test('preserves the authoritative workspace layout across another live chat and history', async ({ launchApp }) => {
  ...
  try {
    ...
    await expect(page.getByLabel('SSH 窗口布局设置')).toBeVisible()
    await page.getByRole('button', { name: '选择任务 聊天 A', exact: true }).click()
    await expect(page.getByRole('button', { name: '选择终端会话 127.0.0.2', exact: true })).toBeVisible()
    await expect(page.locator('[data-testid^="terminal-pane-"]:visible')).toHaveCount(1)
    await expect(page.getByLabel('SSH 窗口布局设置')).toHaveCount(0)
...
test('layout controls persist while all terminal panes remain rendered and scrollable', async ({ launchApp }) => {
  ...
  try {
    ...
    await page.getByRole('button', { name: 'SSH 窗口布局', exact: true }).click()
    const layoutMenu = page.getByLabel('SSH 窗口布局设置', { exact: true })
    await expect(layoutMenu).toBeVisible()
    const layoutMenuBox = await layoutMenu.boundingBox()
    const viewport = await page.evaluate(() => ({ width: window.innerWidth, height: window.innerHeight }))
    expect(layoutMenuBox).not.toBeNull()
    if (layoutMenuBox) {
      expect(layoutMenuBox.y).toBeGreaterThanOrEqual(0)
      expect(layoutMenuBox.y + layoutMenuBox.height).toBeLessThanOrEqual(viewport.height)
    }
...
test('removes per-terminal maximize and defaults to a full-height SSH row', async ({ launchApp }) => {
  const sshServer = await startSshServer()
  let app: ElectronApplication | undefined
...
test('ignores a late direct connection after newer chat navigation and dialog generations', async ({ launchApp }) => {
  ...
  try {
    ...
    const initialChatId = await page.locator('.history-item.active').getAttribute('data-chat-id')
    if (!initialChatId) throw new Error('Expected initial task id')
    await page.getByRole('button', { name: '新建任务', exact: true }).click()
    const newerChatId = await page.locator('.history-item.active').getAttribute('data-chat-id')
    if (!newerChatId) throw new Error('Expected newer task id')
    ...
    await chatItem(page, newerChatId).getByRole('button', { name: /^选择任务 / }).click()
...
test('renders a structured Ollama reply and restores it after reload', async ({ launchApp }) => {
  ...
  try {
    ...
    const chatId = await createNamedChat(page, 'AI 聊天 E2E')
    ...
    await expect(aiWorkspace.getByRole('button', { name: '重试', exact: true })).toHaveCount(0)
    await expect(aiWorkspace.getByRole('button', { name: /再次执行|执行此命令|自动驾驶/ })).toHaveCount(0)

    await page.reload()
    await waitForWorkbenchReady(page)
    await chatItem(page, chatId).getByRole('button', { name: '选择任务 AI 聊天 E2E', exact: true }).click()
    await expect(aiWorkspace.locator('.message.user')).toContainText('第一条消息')
    await expect(aiWorkspace.locator('.message.assistant')).toContainText('你好，世界')
    expect(fakeModel.chatRequests).toBeGreaterThanOrEqual(1)
  } finally {
    await app?.close()
...
test('supersedes a streaming global AI chat from the same input and retains only the second completion', async ({ launchApp }) => {
  ...
  try {
    ...
    await page.getByRole('button', { name: '发送', exact: true }).click()
    await expect(page.locator('.progress-item')).toContainText('正在思考')
    await expect(input).toBeEnabled()
    ...
    expect(assistantMessages).toEqual([
      ...
      expect.objectContaining({ content: '{"version":1,"reply":"第二条响应完成","plan":null}', state: 'complete' }),
    ])
...
test('authoritatively cancels global AI before deleting its chat', async ({ launchApp }) => {
  ...
  try {
    ...
    const chatId = await createNamedChat(page, '待删除 AI 聊天')

    await page.getByLabel('聊天输入').fill('删除前取消')
    await page.getByRole('button', { name: '发送', exact: true }).click()
    await expect(page.getByRole('button', { name: '取消', exact: true })).toBeVisible()
    await chatItem(page, chatId).getByRole('button', { name: '任务操作 待删除 AI 聊天', exact: true }).click()
    await chatItem(page, chatId).getByRole('menuitem', { name: '删除', exact: true }).click()
...
test('clearing acknowledged host memory persists removal and requires fresh SSH disclosure after restart', async ({ launchApp }) => {
  ...
  try {
    ...
    await test.step('reconnects the same endpoint through the visible SSH form', async () => {
      ...
      await restartedPage.getByRole('button', { name: '关闭 SSH 会话 127.0.0.1', exact: true }).click()
...
test('selects an unowned startup session over persisted history', async ({ launchApp }) => {
  ...
  try {
    ...
    const historyTitle = (await page.locator('.history-item.active .chat-title > strong').textContent())?.trim()
...
function chatItem(page: Awaited<ReturnType<ElectronApplication['firstWindow']>>, chatId: string): Locator {
  return page.locator(`.history-item[data-chat-id="${chatId}"]`)
}
...
async function removeActiveTask(page: Awaited<ReturnType<ElectronApplication['firstWindow']>>): Promise<void> {
  ...
  // Deleting a live task first transfers its SSH sessions, then removes the source task.
...
async function startFakeOllamaServer(): Promise<{ server: HttpServer; port: number; requests: number; connectionTests: number; chatRequests: number }> {
  ...
  const server = createHttpServer((request, response) => {
    ...
    request.on('end', () => {
      ...
      const content = JSON.stringify({ version: 1, reply: '你好，世界', plan: null })
      const parts = [...content]
...
async function startSupersedingOllamaServer(): Promise<{ server: HttpServer; port: number; release: () => void; requests: number; firstAborted: number }> {
  ...
  const server = createHttpServer((request, response) => {
    ...
    response.write(JSON.stringify({ message: { content: JSON.stringify({ version: 1, reply: '第二条响应完成', plan: null }) } }) + '\n')
    response.end(JSON.stringify({ done: true }) + '\n')
  })
  server.listen(0, '127.0.0.1')
  await once(server, 'listening')
