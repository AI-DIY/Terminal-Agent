...
async function connect(request: Parameters<typeof window.terminalAgent.sessions.connect>[0]): Promise<void> {
  connectionError.value = ''
  try {
    const session = await window.terminalAgent.sessions.connect(request)
    addSession(session)
...
.connect-button,.upgrade-button,.settings-button { margin: 12px; padding: 8px 12px; border: 0; border-radius: 5px; background: #0284c7; color: white; }.upgrade-button{background:#166534}.settings-button{background:#475569}
