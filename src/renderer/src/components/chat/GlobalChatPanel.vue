...
import { Bot, Check, CircleAlert, PanelRightClose, Pencil, Send, Square, Trash2, UserRound, X } from '@lucide/vue'
          ...
          <section v-if="message.executionPlan" class="execution-plan" :data-status="message.executionPlan.status">
            ...
            <div v-for="step in message.executionPlan.steps" :key="step.id" class="plan-step">
              ...
              <div v-if="message.executionPlan.status === 'pending_review' && !readOnly" class="plan-step-actions">
                <input class="plan-edit-input" :value="stepCommand(step)" :aria-label="`编辑 ${step.target} 命令`" @change="editStep(message.id, step.id, ($event.target as HTMLInputElement).value)">
                <button type="button" class="icon-button" :aria-label="`保存 ${step.target} 命令`" title="保存命令" @click="editStep(message.id, step.id, stepCommand(step))"><Check :size="13" aria-hidden="true" /></button>
