...
/**
 * Keep the task-facing status independent from the old SSH-count metadata.
 * A freshly-created task has titleState `new` and no associations; once a
 * message or SSH association exists it is a completed/paused task unless it
 * is currently represented in the live-chat set.
 */
function taskStatus(chat: ChatSummary): string {
  if (props.onlineChatIds?.has(chat.id)) return '正在工作'
  if (chat.titleState === 'new' && chat.shellCount === 0) return '未开始'
  return '已结束'
}
...
<template>
  <aside class="session-sidebar" aria-label="任务空间">
    <header class="sidebar-header">
      <div class="panel-title"><strong>任务空间</strong><span>任务与 SSH 记录</span></div>
      <button type="button" class="collapse-button" aria-label="收起任务空间" title="收起任务空间" @click="emit('collapse')"><PanelLeftClose :size="14" aria-hidden="true" /><span>收起</span></button>
    </header>
      ...
      <section v-for="group in groups" :key="group.label" class="chat-group" :aria-label="group.label">
        ...
        <div v-for="chat in group.chats" :key="chat.id" class="history-item" :data-chat-id="chat.id" :class="{ active: chat.id === currentChatId, 'has-online-shell': props.onlineChatIds?.has(chat.id) }">
          ...
          <button v-else type="button" class="chat-select" :aria-label="`选择任务 ${chat.title}`" :aria-current="chat.id === currentChatId ? 'page' : undefined" @click="emit('select', chat.id)">
            ...
            <span class="chat-select-meta"><span>{{ createdLabel(chat) }} 创建</span><strong :class="{ 'working-status': taskStatus(chat) === '正在工作', 'finished-status': taskStatus(chat) === '已结束' }">{{ taskStatus(chat) }}</strong></span>
          </button>
