...
import { testRegexFenceRules } from './regex-fence-rule-tester'
...
function add(){rules.value.push({id:`rule-${Date.now()}`,name:'新规则',pattern:'',enabled:true})} function test(){testResult.value=testRegexFenceRules(rules.value,tester.value)} async function save(){try{await window.terminalAgent.settings.saveRegexRules(rules.value);message.value='已保存'}catch(e){message.value=e instanceof Error?e.message:'保存失败'}}
</script>
