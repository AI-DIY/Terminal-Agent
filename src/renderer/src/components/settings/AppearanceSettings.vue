  ...
  <section class="settings-panel appearance" aria-labelledby="appearance-title">
    <header class="panel-head">
      <div>
        ...
        <p>选择工作台主题。布局、侧栏宽度和 SSH 排列方式会自动记忆，所有连接均可在工作区内滚动查看。</p>
      </div>
    ...
    <section class="theme-band" aria-label="工作台主题">
      <div class="theme-options" role="group" aria-label="工作台主题">
        ...
        <button type="button" :class="{ selected: layout.state.theme === 'imperial-gold' }" :aria-pressed="layout.state.theme === 'imperial-gold'" @click="saveTheme('imperial-gold')"><span class="theme-swatch imperial-gold" aria-hidden="true"><i /><i /></span><span><strong>帝王金</strong><small>黑曜底色上的香槟金高光与金属层次</small></span></button>
        <button type="button" :class="{ selected: layout.state.theme === 'sakura-pink' }" :aria-pressed="layout.state.theme === 'sakura-pink'" @click="saveTheme('sakura-pink')"><span class="theme-swatch sakura-pink" aria-hidden="true"><i /><i /></span><span><strong>樱花粉</strong><small>柔和明亮的粉色层次，轻盈易读</small></span></button>
        <button type="button" :class="{ selected: layout.state.theme === 'jasmine-green-tea' }" :aria-pressed="layout.state.theme === 'jasmine-green-tea'" @click="saveTheme('jasmine-green-tea')"><span class="theme-swatch jasmine-green-tea" aria-hidden="true"><i /><i /></span><span><strong>茉绿茶</strong><small>清润茶绿与墨色层次，雅致舒展</small></span></button>
      </div>
    </section>
...
<style scoped>
.appearance { display: grid; gap: 0; max-width: 1080px; overflow: hidden; }
...
.theme-band { padding: 8px 2px 20px; }.theme-options { display: grid; grid-template-columns: repeat(auto-fit, minmax(min(100%, 150px), 1fr)); gap: 14px; max-width: 1080px; }
...
.theme-swatch.imperial-gold { border-color: #8a7140; background: #191714; }.theme-swatch.imperial-gold::before { border-color: #5d4d2c; background: #242019; }.theme-swatch.imperial-gold::after { border-color: #b99755; background: #0d0c0a; box-shadow: inset 0 12px 0 #211d16; }.theme-swatch.imperial-gold i:first-child { border-color: #4a3e28; background: #161411; }.theme-swatch.imperial-gold i:last-child { background: #12110f; }
.theme-swatch.sakura-pink { border-color: #e1a9bd; background: #fff0f4; }.theme-swatch.sakura-pink::before { border-color: #e8bdcb; background: #f8dfe8; }.theme-swatch.sakura-pink::after { border-color: #e8b6c7; background: #fff8fa; box-shadow: inset 0 12px 0 #fff1f5; }.theme-swatch.sakura-pink i:first-child { border-color: #e7bfce; background: #fbe4ec; }.theme-swatch.sakura-pink i:last-child { background: #fff8fa; }
.theme-swatch.jasmine-green-tea { border-color: #abc7af; background: #eef6ec; }.theme-swatch.jasmine-green-tea::before { border-color: #c4d9c4; background: #e5f0e6; }.theme-swatch.jasmine-green-tea::after { border-color: #b6cfb8; background: #fbfdf8; box-shadow: inset 0 12px 0 #f2f7ed; }.theme-swatch.jasmine-green-tea i:first-child { border-color: #c5d8c5; background: #e8f1e7; }.theme-swatch.jasmine-green-tea i:last-child { background: #fbfdf8; }
...
@media (max-width: 760px) { .settings-band { grid-template-columns: 1fr; gap: 14px; }.layout-preview { grid-column: 1; }.layout-fields { grid-template-columns: 1fr; } }
</style>
