  ...
  <section class="settings-panel appearance" aria-labelledby="appearance-title">
    ...
    <section class="theme-band" aria-label="工作台主题">
      <div class="theme-options" role="group" aria-label="工作台主题">
        ...
        <button type="button" :class="{ selected: layout.state.theme === 'imperial-gold' }" :aria-pressed="layout.state.theme === 'imperial-gold'" @click="saveTheme('imperial-gold')"><span class="theme-swatch imperial-gold" aria-hidden="true"><i /><i /></span><span><strong>帝王金</strong><small>璀璨亮金的深色层次，铺陈华贵高调质感</small></span></button>
...
.theme-band { padding: 8px 2px 20px; }.theme-options { display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 14px; max-width: 1080px; }
...
.theme-swatch.imperial-gold { border-color: #d7af3e; background: #3b2a04; }.theme-swatch.imperial-gold::before { border-color: #b57c0c; background: #c99419; }.theme-swatch.imperial-gold::after { border-color: #e1c15c; background: #160f02; box-shadow: inset 0 12px 0 #3b2a04; }.theme-swatch.imperial-gold i:first-child { border-color: #806015; background: #2c2005; }.theme-swatch.imperial-gold i:last-child { background: #211805; }
...
@media (max-width: 960px) { .theme-options { grid-template-columns: repeat(3, minmax(0, 1fr)); } }
@media (max-width: 760px) { .settings-band { grid-template-columns: 1fr; gap: 14px; }.layout-preview { grid-column: 1; }.layout-fields { grid-template-columns: 1fr; }.theme-options { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
