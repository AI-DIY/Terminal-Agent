...
export function parseAccessClientArgv(argv: readonly string[]): AccessClientInvocation {
  ...
  if (args[0] === '-load' && (args.length === 2 || (args.length === 4 && args[2] === '-pw'))) {
    ...
    if (!target || (args.length === 4 && !password)) throw new Error('Unsupported AccessClient invocation')
    if (target.startsWith('tmp:')) {
      const path = target.slice(4)
      if (!path) throw new Error('Unsupported AccessClient invocation')
      return { kind: 'temporary-session', path, ...(password ? { password } : {}) }
    }
    if (password) throw new Error('Unsupported AccessClient invocation')
    return { kind: 'saved-session', name: target }
  }

  if (args.length === 3 && args[0] === '-raw' && args[1] === '-P') {
    const port = parsePort(args[2])
    return { kind: 'raw-port', port }
  }

  throw new Error('Unsupported AccessClient invocation')
}
