...
> 本次 1.0.3 按用户要求以最基础、最快可交付的范围实现：双击入口、管理员授权、显式确认、备份后删除、说明文件、ZIP 打包和发布验证。复杂的注册表依赖注入和真实删除集成测试留给后续需要时再扩展。
...
脚本作为程序运行时只使用标准 Windows 卸载路径。1.0.3 的自动化验证使用源码断言、PowerShell 语法检查和 `-WhatIf` 只读预览，不触碰用户真实卸载条目。
...
自动化验证检查 PowerShell 语法、`-WhatIf -NoPause` 只读预览和 ZIP 内容。预览可以枚举真实卸载列表，但不会导出、删除或重命名任何注册表项；真实删除仍必须由用户在提升后的交互窗口输入 `Y` 完成。
