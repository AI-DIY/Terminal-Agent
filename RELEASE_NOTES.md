...
# Terminal-Agent v3.3.3

Release date: 2026-09-10

## 开源 TA 修改意见微调

- 新增标准 Dynamic Skill 工作流：应用会发现项目根目录 `.skills/<skill-name>/SKILL.md`，校验 YAML 头部并显示诊断；支持刷新、启停、重启后保留启用状态、`$` 搜索和多选，以及按需加载技能正文和附属文件。
- AI 可以在用户明确选择或模型按说明请求时，受控地加载标准 Skill、读取其附属文件并执行本机命令。命令固定在 Skill 目录中运行，具备超时、取消、输出上限、失败回传和临时执行状态卡片；安装版会将 `.skills` 放在 EXE 同级而不是 `app.asar` 内。
- SSH 广播栏把“上传到所有会话”放在“发送所有会话执行”右侧：一次选择本地文件后，向全部已连接 SSH 会话各自的用户目录并发上传，并分别反馈成功、失败或取消状态。
- 文件传输全屏模式为 Electron 原生标题栏预留安全区域，右上角的面板操作不会再和窗口控件冲突。
- 新建 SSH 连接的嵌入页、历史连接页和弹窗统一支持内容区滚动；滚动条默认隐去，仅在悬停或键盘焦点进入时出现。
- 收紧 AI 工作区的消息、思考、计划、执行结果与上下文间距，同时保持展开上下文的可读字号。应用启动不再无条件建立空聊天，重启后会恢复已保存的聊天和原工作空间。

## Windows 发布包

- `Terminal-Agent-Setup-3.3.3.exe`
- `putty.exe`
- `快速安装脚本.cmd`
- `快速安装手册.md`
- `快速安装手册.pdf`
- `Terminal-Agent-Quick-Install-3.3.3.zip`
- `Terminal-Agent-Uninstall-Cleanup-3.3.3.zip`

---

# Terminal-Agent v3.3.2

Release date: 2026-09-09

## 开源 TA 修改意见微调

- AI 执行计划按 SSH 会话并发调度：不同会话可以同时执行，同一会话内仍保持步骤顺序；只有全部会话完成或失败后才进入下一步分析，单个会话失败不会取消其他会话。
- SSH 工作区广播文案统一为“发送命令到所有会话”和“发送所有会话执行”，避免将在线会话误称为窗口。
- 新增“上传文件到所有会话”：本机文件只选择一次，并行上传到所有在线 SSH 会话的用户目录；每个会话独立显示成功、失败和失败原因。
- 文件传输面板支持全屏、上下拖动调整高度以及隐藏/折叠而不销毁；传输表补充状态、进度、大小、本地路径、方向、远程路径、速度、预计剩余时间和经过时间等信息。
- 新建 SSH 页面优化滚动条、最大化居中和认证方式布局：窄屏时用户名密码/私钥表单动态适配，移除冗余连接方式说明，并改善间距与颜色层次。
- AI 工作区聊天卡片、计划卡、思考状态和上下文留白进一步压缩；展开后的上下文内容字号保持不变。已有聊天数据在应用重启后切换回对应工作空间时完整恢复。

## Windows 发布包

- `Terminal-Agent-Setup-3.3.2.exe`
- `putty.exe`
- `快速安装脚本.cmd`
- `快速安装手册.md`
- `快速安装手册.pdf`
- `Terminal-Agent-Quick-Install-3.3.2.zip`
- `Terminal-Agent-Uninstall-Cleanup-3.3.2.zip`

---

# Terminal-Agent v3.3.1

Release date: 2026-09-09

## 开源 TA 修改意见微调

- 设置导航暂时隐藏“安全围栏”，保留已有规则、配置、IPC 和运行时安全约束；外观新增“茉绿茶”主题，并适配任务空间、SSH 工作区、AI 工作区及原生标题栏。
- SSH 文件传输改为在既有 SSH transport 上串行复用一个持久 SFTP 子系统；失败时只废弃 SFTP 子通道，不关闭交互式终端。连接与 SFTP 生命周期会写入安装目录 `logs\\ssh-connection.log`，记录不包含密码、私钥或命令内容。
- “发送命令到所有窗口”输入框补充组合键发送提示，并新增放大编辑图标和大空间编辑弹窗；保留普通 Enter 发送、组合键直接发送和 Shift+Enter 多行编辑行为。
- 移除 SSH 工作区的“返回实时任务”入口；所有新建 SSH 连接场景支持固定认证方式头部、内容垂直滚动和窄窗口文字换行，不再裁切下方内容。
- AI 工作区的普通助手文本统一采用安全 Markdown 渲染；思考状态、审核命令计划和已执行命令卡片保持原有专用呈现。

## Windows 发布包

- `Terminal-Agent-Setup-3.3.1.exe`
- `putty.exe`
- `快速安装脚本.cmd`
- `快速安装手册.md`
- `快速安装手册.pdf`
- `Terminal-Agent-Quick-Install-3.3.1.zip`
- `Terminal-Agent-Uninstall-Cleanup-3.3.1.zip`
...
- 新建 SSH 连接的空状态、仅历史连接状态和连接弹窗统一为双栏连接工作台：左侧清晰选择堡垒机、密码或私钥方式，右侧保留原有表单、配置须知和使用截图；所有既有交互、键盘导航和焦点管理保持不变。
- AI 工作区更新消息卡片、上下文标签、计划确认和输入区层级；右侧 AI 面板可向左拖到应用窗口约 40% 宽，并会随窗口尺寸自动适配。
- 单点登录在原应用窗口中完成认证后，自动恢复、显示并聚焦主窗口；“重试”和“打开单点登录设置”及原有认证流程未改变。
- 重做帝王金主题为黑曜底色、香槟金高光和克制的金属层次，同步更新原生窗口标题栏与主题预览。
- SSH 重名角标优先依据真实目标主机名，不再把共享堡垒机 IP 误判为同一主机；文件传输为同一 SSH transport 串行化 SFTP 子系统请求并启用保活，避免打开传输面板后意外断开主 SSH。
- SSH 画布与空状态使用对称的左右安全留白，滚动区域也保留对称空间。

## Windows 发布包

- `Terminal-Agent-Setup-3.2.9.exe`
- `putty.exe`
- `快速安装脚本.cmd`
- `快速安装手册.md`
- `快速安装手册.pdf`
- `Terminal-Agent-Quick-Install-3.2.9.zip`
- `Terminal-Agent-Uninstall-Cleanup-3.2.9.zip`
...
- 大语言模型（含 API Key）与单点登录配置统一保存至 `%USERPROFILE%\.terminal-agent\user-config`；已有文件绝不覆盖，并可在首次启动时从旧版 `.ta\user-config` 迁移，同时保留未知配置字段。
- 外观新增“帝王金”和“樱花粉”，并同步适配任务空间、SSH 工作区、AI 工作区及原生窗口标题栏。
- SSH 工作区底部新增唯一的“发送键输入到所有会话”控件：通过显式开关启用，关闭时输入与发送不可用，发送结果会反馈成功/失败会话数量。
- SSH 工作区新增可隐藏但不会中断进行中任务的文件传输面板，支持上传、下载、远端目录浏览、右键操作、拖拽操作、实时进度与传输记录；关闭 SSH 后仍可在对应 Shell 历史中查看文件传输日志。
- 升级流程以 `http://ta.ai-diy.me/update/win32` 的 Nuts 服务为版本依据，避免重复操作并以非阻塞方式显示可理解的失败信息。
- 技能页精简为内置技能目录和启用开关，移除欢迎、姓名、预览、本地保存等无关展示内容。
- Shell 历史移除界面上的重新连接操作及说明；每次连接详情新增“SSH连接操作记录”和“文件传输日志”两个可切换标签页。

## Windows 发布包

- `Terminal-Agent-Setup-3.2.7.exe`
- `putty.exe`
- `快速安装脚本.cmd`
- `快速安装手册.md`
- `快速安装手册.pdf`
- `Terminal-Agent-Quick-Install-3.2.7.zip`
...
- 快速安装手册改为通用版本占位符并补充 AccessClient 会话配置和集团堡垒机使用截图；快速安装 ZIP 同时包含 Markdown 文本版和由 Markdown 动态生成的 `快速安装手册.pdf` 打印版。
...
- AI 工作区暂时隐藏“立即压缩”，保留现有后端兼容能力；执行计划命令改为可自动换行的多行编辑框，确认执行时以用户当前输入为准。
- 明确勾选多个 SSH 主机时，规划器会把计划限制在所选主机，并为每个主机生成独立步骤，避免只执行第一台机器。
- 设置导航移除“模型选择”和“视觉语言模型配置”，统一使用大语言模型激活配置；模型配置与明文 API Key 统一保存到 `%USERPROFILE%\.ta\user-config`。
- 外观新增“高贵紫”主题；SSH 字体大小和窗口行高各扩展两档，并同步到终端窗口。
- 任务历史区更名为“任务空间”，SSH 历史连接更名为“SSH 工作区”；新建 SSH 弹窗与工作区入口共用同一组件，默认堡垒机 SSH 连接并显示配置/使用须知。
- 单点登录的远程认证窗口改为后台渲染，前台登录页显示本地“登录进行中”动画，认证完成后再进入工作台。
- 升级检查接入 Nuts 服务 `http://ta.ai-diy.me/update/win32`，支持 204/200 响应、安装包安全校验和最多 4 路 Range 并发下载。

## Windows 发布包

- `Terminal-Agent-Setup-3.2.0.exe`
- `putty.exe`
- `快速安装脚本.cmd`
- `快速安装手册.md`
- `快速安装手册.pdf`
- `Terminal-Agent-Quick-Install-3.2.0.zip`
