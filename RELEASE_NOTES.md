# Terminal-Agent v3.2.6

Release date: 2026-09-06

## 开源 TA 修改意见微调

- 技能页更新为系统告警分析、系统日报周报月报分析和系统知识库检索三个运维平台内置技能，默认停用并标注为演示项；底层数据源尚未接入。
- AI 工作区移除“新建 SSH 连接”入口，SSH 工作区保留独立的连接入口。
- 执行计划确认后逐条等待目标 SSH 命令完成，再将完整返回结果交给后续 AI 分析，避免提前读取不完整上下文。
- “正在思考”改为左侧 AI 状态项，使用动态点阵动画；AI 开始输出计划或文字内容时自动清除，并在执行计划后的自动分析阶段重新显示。

## Windows 发布包

- `Terminal-Agent-Setup-3.2.6.exe`
- `putty.exe`
- `快速安装脚本.cmd`
- `快速安装手册.md`
- `快速安装手册.pdf`
- `Terminal-Agent-Quick-Install-3.2.6.zip`
